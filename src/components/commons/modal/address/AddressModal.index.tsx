import { useInput, useInputWithMaxLength, useInputWithRegex } from "@/src/lib/hooks/useInput";
  ...
  return (
    <Modal isOpen={props.isOpen} onClose={props.onClose}>
      <S.Wrapper>
        ...
        <S.Input placeholder="배송지명을 입력하세요" value={nameArgs.value} onChange={nameArgs.onChange} maxLength={20}/>
        ...
        <Spacer width="100%" height="10px" />
        <S.Input placeholder="이름을 입력하세요" value={receiverArgs.value} onChange={receiverArgs.onChange} maxLength={10}/>
        <Spacer width="100%" height="24px" />
        <div className="flex-row">
        ...
        <Spacer width="100%" height="10px" />
        <S.AddressInputWrapper className="flex-row-between-center" onClick={openPostPopup}>
          <S.AddressInput placeholder="우편 번호" value={zoneCode} readOnly/>
          <S.SearchButton>검색</S.SearchButton>
        </S.AddressInputWrapper>
        <Spacer width="100%" height="5px" />
        <S.Input placeholder="주소" onClick={openPostPopup} value={address} readOnly/>
        <Spacer width="100%" height="5px" />
        ...
        <Spacer width="100%" height="10px" />
        <S.Input placeholder="휴대폰 번호 (숫자만)" value={phone1} onChange={onChangePhone1}/>
        <Spacer width="100%" height="24px" />
        <S.Label className="medium16">연락처2</S.Label>
        <Spacer width="100%" height="10px" />
        <S.Input placeholder="휴대폰 번호 (숫자만)" value={phone2} onChange={onChangePhone2}/>
        <Spacer width="100%" height="24px" />
        <S.CheckArea className="flex-row-align-center" onClick={() => setDefaultCheck(!defaultCheck)}>
          <CheckIcon isChecked={defaultCheck} size={24} defaultColor="var(--color-normalGray)"/>
          <S.CheckLabel isChecked={defaultCheck} className="regular14">기본 배송지로 설정</S.CheckLabel>
        </S.CheckArea>
        <Spacer width="100%" height="30px" />
        <div className="flex-row">
          ...
          <S.SubmitButton className="bold16" isActive={submitAvailable} onClick={onSubmit} disabled={!submitAvailable}>
            등록하기
          </S.SubmitButton>
        </div>
      </S.Wrapper>
